  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
      <motion.nav
        ...
        className="flex items-center gap-1 p-2 bg-background/50 backdrop-blur-xl border border-border border-dashed rounded-2xl shadow-2xl transition-colors duration-300"
      >
        {items.map((item, idx) => (
          <Link
            key={idx}
          ...
          >
            ...
            {/* Tooltip */}
            <span className="absolute -top-12 left-1/2 -translate-x-1/2 px-2 py-1 bg-background border border-border border-dashed text-muted text-[10px] font-mono uppercase tracking-widest rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap shadow-xl">
              {item.label}
            </span>
          </Link>
        ))}
